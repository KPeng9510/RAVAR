...
def eval_epoch(args, model, test_dataloader, device, n_gpu, test):
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader): # Maybe something went wrong here!!!
            index, key_frame, iids, input_mask, segment_ids, video, video_mask, bbox, anns = batch
            ...
            for sids, imask, v, vm, ann in zip( segment_ids.transpose(0,1), input_mask.transpose(0,1), video.transpose(0,1), video_mask.transpose(0,1), anns.transpose(0,1)):
                #print(v.shape)
                loss, predictions, prediction_bboxes, ann, logits = model(key_frame, iids, sids, imask, v, vm, bbox, ann, training=False)
                ...
                b_boxes.append(prediction_bboxes)
            all_gt_boxes.append(gather(bbox).cpu().numpy())
            all_iou.append(np.mean(batch_iou))
            b_pred = torch.stack(b_pred).mean(0)
            b_logits = torch.stack(b_logits).mean(0)
            b_boxes = torch.stack(b_boxes).mean(0)
            b_pred = gather(b_pred)
            b_logits = gather(b_logits)
            b_pred_ann = gather(ann)
            b_boxes = gather(b_boxes)
            score_list.append(b_pred.cpu().numpy())
            label_list.append(b_pred_ann.cpu().numpy())
            logits_list.append(b_logits.cpu().numpy())
            boxes_list.append(b_boxes.cpu().numpy())
        ...
        boxes_list = np.concatenate(boxes_list, 0)
        index_list = np.concatenate(index_list, 0)
        gt_box_list = np.concatenate(all_gt_boxes, 0)
        dict_all = {}
        dict_all["score"] = score_list
        dict_all["label"] = label_list
        dict_all["logits"] = logits_list
        dict_all["boxes"] = boxes_list
        dict_all["indexes"] = index_list
        dict_all["gt_boxes"] = gt_box_list
        import pickle as pkl
        f = open("prediction_ourmodel_supp.pkl", "wb")
        pkl.dump(file=f, obj=dict_all)
        f.close()
...
def get_iou(bb1, bb2):
    ...
    bb2 : dict
        Keys: {'x1', 'x2', 'y1', 'y2'}
        The (x, y) position is at the top22222 left corner,
        the (x2, y2) position is at the bottom right corner
...
def main():
    ...
    if args.do_train:
        ...
        R2 = eval_epoch(args, model, test_dataloader, device, n_gpu, test='TEST')
