...
class AgentAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
        ...
        #self.agent_dropout = nn.Dropout(0.2)
        ...
        self.pool = nn.Sequential(nn.Linear(dim, int(dim//2)), nn.GELU(), nn.Dropout(0.2), nn.Linear(int(dim//2), self.num_heads*self.agent_num*dim))
...
class Blipv2(nn.Module):
    def __init__(self,):
        ...
        self.proj =nn.Sequential(nn.Linear(772,128), nn.GELU(), norm.SelfNorm(128), nn.Linear(128, 768))
        ...
        self.proj_box = nn.Sequential(nn.Linear(100,128), nn.GELU(), norm.SelfNorm(128), nn.Linear(128, 32))
        self.proj_sem = nn.Sequential(nn.Linear(1,128), nn.GELU(), norm.SelfNorm(128), nn.Linear(128, 32))
