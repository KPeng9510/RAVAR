...
class Blipv2(nn.Module):
    ...
    def forward(self, key_frame, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None, training=True):
        ...
        #print(boxes.shape)
        #print(boxes.shape)
        boxes, attn_box= self.agent_box(boxes)
        features_text = self.proj_sem(features_text.contiguous().permute(0,2,1)).contiguous().permute(0,2,1)
        features_text,attn_sem = self.agent_semantic(features_text)
        ...
        features_image = features_image.image_embeds.contiguous().view(b, bs,ts, -1, 768).mean(1)

        features_image,_ = self.agent_temporal(features_image.mean(1), attn_sem, attn_box)
