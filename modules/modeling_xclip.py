...
class XCLIP(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.bbox_regressio_head = nn.Sequential(nn.Linear(512,4*8), nn.Sigmoid())
        self.classification_head = nn.Linear(512, 80)

        self.bceloss = torch.nn.BCEWithLogitsLoss()
        self.mseloss = torch.nn.MSELoss()
        self.apply(self.init_weights)

    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None, bbox=None, ann=None):
        ...
        #print(ann.shape)
        loss_cls = self.bceloss(predictions, ann.float())
        #loss = outputs.loss
        batch_size = bbox_prediction.shape[0]

        #print(bbox.shape)
        loss_bbox = self.mseloss(bbox_prediction.reshape(batch_size,8, 4), bbox.float())
        loss = torch.mean(loss_bbox) + torch.mean(loss_cls)
        return loss, predictions, bbox_prediction
