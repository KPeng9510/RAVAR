...
from dataloaders.ravar_dataset import RAVARDataset

def dataloader_ravar_train(args, tokenizer, subset="train"):
    ravar_dataset = RAVARDataset(
        training='train',
    )
    train_sampler = torch.utils.data.distributed.DistributedSampler(ravar_dataset)
    ...
    return dataloader, len(ravar_dataset), train_sampler

def dataloader_ravar_test(args, tokenizer, subset="val"):
    ravar_dataset = RAVARDataset(
        training='test',
    )
    train_sampler = torch.utils.data.distributed.DistributedSampler(ravar_dataset)
    ...
    return dataloader, len(ravar_dataset)
def dataloader_ravar_val(args, tokenizer, subset="val"):
    ravar_dataset = RAVARDataset(
        training='val',
    )
    train_sampler = torch.utils.data.distributed.DistributedSampler(ravar_dataset)
...
DATALOADER_DICT = {}
DATALOADER_DICT["ravar"] = {"train":dataloader_ravar_train, "val":dataloader_ravar_val, "test":dataloader_ravar_test}
